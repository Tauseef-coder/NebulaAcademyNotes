...
function funcFunc(){
    ...
    items.forEach(num => copy.push(num + num + 2));
    return copy
}
...
//.map
//. definiton : the map () method creates a new array populated with the results of calling a provice

let numArray =[1,2,3,4,5,9,10]

funciton multiplyElements(array){
    return array.map(x => x * 2)
}
console.log(multiplyEleemnts(numArray))
...
///use HOF divide the elements/currentValue with te index


let newArray[1,2,3,4];

repeat(5, i => {
  newArray.push(`Unit ${i + 1}`);
});
