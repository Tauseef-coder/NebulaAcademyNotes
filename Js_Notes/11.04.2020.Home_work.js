...
function noPulp(orangeJuice){
    if(typeof noPul==='String'){
        return noPulp
    }
    {
        if (typeof noPulp ==='Number'){
            return noPulp
        }
    }   if(typeof noPulp ==='boolean'){
        return noPulp
    }
        if(Array.isArray(noPulp)){
            return noPulp
        }
}
...
console.log(noPulp(['Monday','Tuesday']))
...
function calculate(dismiss){
    if (dismiss.lenght %2 ==1){
        return `the lenght of the srting${dismiss} is an odd number`
    }
}
...
let germanCar= ('Porshe','Volkswagen','Bmw','Audi','MercedesBenz','Alpina')

function isEven(germanCars){
    return germanCar.pop([-1])
}
console.log(germanCars[-1]);
germanCar.push('opel');
germanCar();
